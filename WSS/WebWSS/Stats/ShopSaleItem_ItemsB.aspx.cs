...
{
    public partial class ShopSaleItem_ItemsB : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"
SELECT     F_ItemExcelID, SUM(F_ItemSaleNum / F_ItemChildNum) AS F_GoodsNum, SUM(F_ItemSaleNum) AS F_ItemSaleNum, SUM(F_TradeCount) AS F_TradeCount, SUM(F_RoleCount) 
                      AS F_RoleCount, SUM(F_MoneyCount) AS F_MoneyCount
FROM         T_ShopSaleItem with(nolock)
...
where F_ShopType=1 AND F_Date>='" + searchdateB.ToString(SmallDateTimeFormat)   + "' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat)   + "' ";
            ...
            sql += " GROUP BY F_ItemExcelID order by F_ItemExcelID";
