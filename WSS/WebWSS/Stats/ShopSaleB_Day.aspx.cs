...
{
    public partial class ShopSaleB_Day : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"
SELECT     F_BigZone, F_ZoneID,F_Date, SUM(F_TradeCount) AS F_TradeCount, SUM(F_RoleCount) AS F_RoleCount, SUM(F_MoneyCount) AS F_MoneyCount
...
where F_ShopType=1 AND F_Date>='" + searchdateB.ToString(SmallDateTimeFormat)   + "' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat)   + "' ";
            ...
            //}
            sql += " GROUP BY F_Date,F_BigZone,F_ZoneID order by F_Date";
