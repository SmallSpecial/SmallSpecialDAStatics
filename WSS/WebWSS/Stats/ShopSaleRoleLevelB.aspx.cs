...
{
    public partial class ShopSaleRoleLevelB : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"
SELECT F_RoleLevel, SUM(F_TradeCount) AS F_TradeCount, SUM(F_RoleCount) AS F_RoleCount, SUM(F_MoneyCount) AS F_MoneyCount
...
where F_ShopType=1 AND F_Date>='" + searchdateB.ToString(SmallDateTimeFormat)   + "' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat)   + "' ";
            ...
            sql += " GROUP BY F_RoleLevel order by F_RoleLevel asc";
