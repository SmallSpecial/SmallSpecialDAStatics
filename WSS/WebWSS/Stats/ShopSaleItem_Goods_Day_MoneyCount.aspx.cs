...
{
    public partial class ShopSaleItem_Goods_Day_MoneyCount : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            if (queryDateView != null && queryDateView.Count > 0)
            {
                ...
                string sql = @"SELECT F_ItemExcelID,F_ItemChildNum," + inSql + "FROM (SELECT F_ItemExcelID,F_ItemChildNum,F_Date, SUM(F_MoneyCount) AS F_MoneyCount FROM T_ShopSaleItem with(nolock) where F_ShopType=0 AND F_Date>='" + searchdateB.ToString(SmallDateTimeFormat) + "' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat) + "' ";

                if (DropDownListArea1.SelectedIndex > 0)
                {
                    sql += @" and F_BigZone=" + DropDownListArea1.SelectedValue.Split(',')[1] + "";
                }
                sql += " GROUP BY F_Date,F_ItemExcelID,F_ItemChildNum) as TempData pivot (sum(F_MoneyCount) for F_Date in (" + inSql + ")) as ourpivot order by F_ItemExcelID";
                try
                {
