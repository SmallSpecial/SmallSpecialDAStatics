...
namespace WebWSS.Stats.Item
{
    public partial class TypeTotal : BasePage
    {
        ...
        public void bind()
        {
            ...
            string sql = @"SELECT F_Date,SUM(ISNULL(F_MoneyCount,0)) as F_MoneyCount FROM T_Item_GSLog_Total with(nolock) where 1=1 " + sqlwhere + " group by  F_Date order by F_Date " + Request.QueryString["order"] + "";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    int num = Convert.ToInt32(e.Row.Cells[1].Text.Replace(",", ""));
                    if (dicSum.ContainsKey(1))
                    {
                        dicSum[1] += num;
                    }
                    else
                ...
                else if (e.Row.RowType == DataControlRowType.Footer)
                {
                    ...
                    e.Row.Cells[1].Text = dicSum[1].ToString("#,#0");
                }
            }
            catch (System.Exception ex)
            {
