...
{
    public partial class PickUpItemRank : BasePage
    {
        ...
        public void bind()
        {
            ...
            {
                sqlwhere += @" and F_PARA_2 =" + Request.QueryString["p2"] + "";
            }
            sqlwhere += " and F_OPID=" + Request.QueryString["opid"] + " and F_PARA_1=" + Request.QueryString["p"] + "";

            string sql = "";
            sql = @"SELECT  top 100 ROW_NUMBER() OVER(order by F_TradeCount desc) AS rownum, F_BigZone,F_ZoneID,F_UserID, F_RoleID, F_TradeCount FROM T_Item_GSLog_Rank with(nolock) where 1=1 " + sqlwhere + " ";
