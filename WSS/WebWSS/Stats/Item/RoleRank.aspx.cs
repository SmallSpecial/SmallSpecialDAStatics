...
{
    public partial class RoleRank : BasePage
    {
        ...
        public void bind()
        {
            ...
            if (Request.QueryString["p2"] != null && Request.QueryString["p2"] != "")
            {
                sqlwhere += @" and F_ExcelID in(" + Request.QueryString["p2"] + ")";
            }
            sqlwhere += " and F_OPID=" + Request.QueryString["opid"] + " and F_PARA_1=" + Request.QueryString["p"] + "";

            string sql = "";
            sql = @"SELECT  top 100 ROW_NUMBER() OVER(order by F_MoneyCount " + Request.QueryString["order"] + ") AS rownum, F_BigZone,F_ZoneID,F_UserID, F_RoleID, F_MoneyCount FROM T_Item_GSLog_Rank with(nolock) where 1=1 " + sqlwhere + " ";
