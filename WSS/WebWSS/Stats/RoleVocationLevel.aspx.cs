...
{
    public partial class RoleVocationLevel : Admin_Page
    {
        string ConnStrDigGameDB = System.Configuration.ConfigurationManager.AppSettings["ConnectionStringDigGameDB"];
        string ConnStrGSSDB = System.Configuration.ConfigurationManager.AppSettings["ConnectionStringGSSDB"];
        DbHelperSQLP DBHelperDigGameDB = new DbHelperSQLP();
        DbHelperSQLP DBHelperGSSDB = new DbHelperSQLP();
        List<long> columns = new List<long>();//总共存在多少列职业角色
        DataSet ds;
...
{0}
 FROM RVcte a order by a.F_Level asc";
            ...
            try
            {
                ...
                columns = new List<long>();//每一次查询都需要清除之前的数据
                foreach (KeyValuePair<string, string> item in MapVacationType())
                {
                    columns.Add(0);
                    jobs.Add(string.Format(template, item.Key, item.Value));
                }
               
                sql = string.Format(sql, string.Join(",", jobs));
                ...
                {
                    if (item.ColumnName == "F_Level")
                    {
                        GridView1.Columns.Add(new BoundField() { DataFormatString = App_GlobalResources.Language.Msg_RoleGradeFormat, DataField = item.ColumnName, HeaderText = App_GlobalResources.Language.LblLevel });
                    }
                    else 
                    {
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                else if (e.Row.RowType == DataControlRowType.Footer)
                {
                    ...
                    {
                        sum += columns[i - 1];
                        e.Row.Cells[i].Text = columns[i-1].ToString("#,#0");
                    }
