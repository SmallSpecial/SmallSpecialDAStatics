...
{
    public partial class ShopSaleB : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"
SELECT     F_BigZone, F_ZoneID, SUM(F_TradeCount) AS F_TradeCount, SUM(F_RoleCount) AS F_RoleCount, SUM(F_MoneyCount) AS F_MoneyCount
...
where F_ShopType=1 AND F_Date>='" + searchdateB.ToString(SmallDateTimeFormat)   + "' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat)   + "' ";
            ...
            //}
            sql += " GROUP BY F_BigZone, F_ZoneID";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.RowIndex >= 0 && !lblerro.Visible)
                {
                    ...
                    sum2 += Convert.ToInt64(e.Row.Cells[3].Text);
                    sum3 += Convert.ToInt64(e.Row.Cells[4].Text.Replace(",", ""));
                }
                else if (e.Row.RowType == DataControlRowType.Footer)
                {
                    e.Row.Cells[0].Text = "";
                    e.Row.Cells[1].Text = App_GlobalResources.Language.LblSum;
                    e.Row.Cells[2].Text = sum1.ToString("#,#0");
                    e.Row.Cells[3].Text = sum2.ToString("#,#0");
                    e.Row.Cells[4].Text = sum3.ToString("#,#0");
                }
            }
            catch (System.Exception ex)
            {
