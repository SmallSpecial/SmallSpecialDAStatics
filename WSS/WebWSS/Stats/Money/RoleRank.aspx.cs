...
namespace WebWSS.Stats.Money
{
    public partial class RoleRank : BasePage
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"SELECT  top 100 ROW_NUMBER() OVER(order by F_MoneyCount " + Request.QueryString["order"] + ") AS rownum, F_BigZone,F_ZoneID,F_UserID, F_RoleID, F_MoneyCount FROM T_Money_GSLog_Rank with(nolock) where 1=1 " + sqlwhere + " ";
