...
{
    public partial class RoleRank_Money_Trade_Count : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            ...
            {
                Common.MsgBox.Show(this, "日期填写不正确,请检查后再试");
                return;
            }
            ...
            string sqlwhere = @" and F_Date='" + searchdateB.ToShortDateString() + "'";
            ...
            if (Request.QueryString["p2"] == "fuzhi")
            {
                sqlwhere += @" and F_PARA_2<0";
            }
            if (Request.QueryString["p2"] == "zhengzhi")
            {
                sqlwhere += @" and F_PARA_2>0";
            }
            ...
            {
                sqlwhere += @" and F_PARA_1=" + Request.QueryString["p"];
            }

            sqlwhere += " and F_OPID=" + Request.QueryString["opid"] + "";

            string sql = "";
            sql = @"SELECT TOP 100 F_BigZone,F_ZoneID,F_UserID, F_RoleID,F_TradeCount,F_MoneyCount FROM T_Money_GSLog_Rank with(nolock) where 1=1 " + sqlwhere + " order by "+Request.QueryString["orderName"]+" " + Request.QueryString["order"];
