...
{
    public partial class TaskFinishRank : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"SELECT top 100 ROW_NUMBER() OVER(order by sum(F_Num) desc) AS rownum, Sum(F_Num) as F_Num, max(F_TaskID) as F_TaskID, max(F_TaskName) as F_TaskName from T_TaskFinishRank where 1=1 " + sqlwhere + "  group by  F_TaskID";
