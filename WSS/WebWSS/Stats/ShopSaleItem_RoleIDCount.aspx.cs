...
{
    public partial class ShopSaleItem_RoleIDCount : Admin_Page
    {
        ...
        public void bind()
        {
            string roleID = GetRoleID(DropDownListArea1.SelectedValue.Split(',')[1],tboxRoleName.Text);
            ...
            string sql = "";
            sql = @"
SELECT top 100 F_ID,F_ZoneID,F_UserID,F_RoleID,F_RoleLevel,F_ItemExcelID,F_TradeCount,F_ShopSaleCount,F_MoneyCount 
FROM T_ShopSaleItemExcelIDCount with(nolock)
where 1=1 AND F_Date='" + searchdateB.ToString(SmallDateTimeFormat)   + "'";
            ...
            if (tboxRoleName.Text != "" && tboxRoleName.Text.Length > 0)
            {
                if (roleID != null && roleID != "")
                {
                    LabelTime.Text = searchdateB.ToString(SmallDateTimeFormat)   + " " + tboxRoleName.Text + "商城消耗";
                    sql += @" and F_RoleID=" + roleID + "";
                }

            }
            sql += "order by F_MoneyCount desc";
        ...
        //得到角色ID
        public string GetRoleID(string bigzone, string roleName)
        {
            try
            {
                DbHelperSQLP spg = new DbHelperSQLP();
                ...
                string sql = @"SELECT top 1 F_RoleID FROM T_BaseRoleCreate with(nolock) WHERE (F_BigZoneID =" + bigzone + ") AND (F_RoleName = '" + roleName + "')  ";
                return string.Format("{0}", spg.GetSingle(sql));
            }
            catch (System.Exception ex)
            {
