...
{
    public partial class TaskBugList : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"SELECT top 100 ROW_NUMBER() OVER(order by sum(F_Num) desc) AS rownum, Sum(F_Num) as F_Num, max(F_TaskID) as F_TaskID, max(F_TaskName) as F_TaskName, max(F_UserID) as F_UserID, max(F_RoleID) as F_RoleID from (select * from T_TaskAcceptRankRT where 1=1 " + sqlwhere + " union all select * from T_TaskFinishRankRT where 1=1 " + sqlwhere + ") a   group by  F_TaskID";
