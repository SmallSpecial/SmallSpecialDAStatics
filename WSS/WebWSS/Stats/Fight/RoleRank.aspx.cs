...
namespace WebWSS.Stats.Fight
{
    public partial class RoleRank : BasePage
    {
        ...
        public void bind()
        {
            ...
            sqlwhere += " and F_OPID=" + Request.QueryString["opid"] + " and F_Type=" + Request.QueryString["p"];

            string sql = "";
            sql = @"SELECT TOP 100 ROW_NUMBER() OVER(order by F_TradeCount "+Request.QueryString["order"]+") as RowNum,F_BigZone,F_ZoneID,F_UserID,F_RoleID,F_TradeCount FROM T_Fight_GSLog_Rank with(nolock) WHERE 1=1 " + sqlwhere + "";
