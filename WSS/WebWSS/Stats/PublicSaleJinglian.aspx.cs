...
{
    public partial class PublicSaleJinglian : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"SELECT      F_Level, sum(F_Num) as F_Num, sum(F_Num0) as F_Num0, sum(F_Num1) as F_Num1, sum(F_Money) as F_Money, sum(F_Money0) as F_Money0, sum(F_Money1) as F_Money1, sum(F_ItemNum) as F_ItemNum, 
                      sum(F_ItemNum0) as F_ItemNum0, sum(F_ItemNum1) as F_ItemNum1
FROM         T_PublicSaleJinglian " + sqlwhere + @" group by F_Level  order by F_Level asc";
