...
{
    public partial class UserLoginTimePara : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"SELECT top 100 SUM(F_UserNum) as F_UserNum, convert(VARCHAR(2),F_TimePara)+' - '+convert(VARCHAR(2),F_TimePara+1) as F_TimeParaT from T_UserLoginTimePara where 1=1 " + sqlwhere + "  group by  F_TimePara order by F_TimePara";
        ...
        private string GetSum(DataView dv)
        {
            int i = 0;
            foreach (DataRow dr in dv.Table.Rows)
            {
                i += Convert.ToInt32(dr["F_UserNum"]);
            }
            return i.ToString();
        }

        public string GetTimePara(object value)
        {
            try
            {
                int i = Convert.ToInt32(value);
                return string.Format("{0} - {1} " + App_GlobalResources.Language.LblUnit_Hour, i, (i + 1));
            }
            catch (System.Exception ex)
            {
                return "";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    double sumi = 0;
                    if (Convert.ToInt64(lblSum.Text) != 0)
                    {
                        sumi = Convert.ToDouble(e.Row.Cells[1].Text) / Convert.ToDouble(lblSum.Text);
                    }

                    e.Row.Cells[2].Text = sumi.ToString("0.00%");
                }
