...
{
    public partial class RoleOnlineIPRank : Admin_Page
    {
        ...
        /// 得到IP地址
        /// </summary>
        public string GetIPLocation(object value)
        {
            try
            {
                DbHelperSQLP sp = new DbHelperSQLP();
                sp.connectionString = ConnStrDigGameDB;
                string sql = @"DECLARE @ip BIGINT;select @ip=dbo.F_IP2int('" + value.ToString().Trim() + "');select top 1 F_Note from T_BaseIPAddress where  F_IPBeginN<= @ip and F_IPEndN>=@ip";
