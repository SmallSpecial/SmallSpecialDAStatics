...
{
    public partial class RoleLoseLevelCreate : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string searchdateB = DateTime.Now.ToString("yyyy-MM-dd");
            string searchdateE = DateTime.Now.ToString("yyyy-MM-dd");
            if (tboxTimeB.Text.Length > 0)
            {
                searchdateB = Convert.ToDateTime(tboxTimeB.Text).ToString("yyyy-MM-dd");
            }
            if (tboxTimeE.Text.Length > 0)
            {
                searchdateE = Convert.ToDateTime(tboxTimeE.Text).ToString("yyyy-MM-dd");
            }
            LabelTime.Text = searchdateB;
            ...
            string sql = "";
            sql = string.Format(@"WITH RVcte(F_Level,F_Pro,F_Num)
                    AS
                    (
                    SELECT F_Level,F_Pro,Sum(Num) AS F_Num FROM 
                    (
                    SELECT CONVERT(VARCHAR(100),F_CreateTime,23) F_CreateTime,F_ZoneID,F_Level,F_Pro,count(1) AS Num FROM 
                    GameCoreDB.dbo.T_RoleBaseData_0 WITH(NOLOCK)
                    WHERE F_UpdateTime<Dateadd(DAY,-3,GETDATE()) AND CONVERT(VARCHAR(100),F_CreateTime,23)>='{0}' AND CONVERT(VARCHAR(100),F_CreateTime,23)<='{1}'
                    group by CONVERT(VARCHAR(100),F_CreateTime,23),F_ZoneID,F_Level,F_Pro
                    UNION ALL
                    SELECT CONVERT(VARCHAR(100),F_CreateTime,23) F_CreateTime,F_ZoneID,F_Level,F_Pro,count(1) AS Num FROM 
                    GameCoreDB.dbo.T_RoleBaseData_1 WITH(NOLOCK)
                    WHERE F_UpdateTime<Dateadd(DAY,-3,GETDATE()) AND CONVERT(VARCHAR(100),F_CreateTime,23)>='{2}' AND CONVERT(VARCHAR(100),F_CreateTime,23)<='{3}'
                    group by CONVERT(VARCHAR(100),F_CreateTime,23),F_ZoneID,F_Level,F_Pro
                    UNION ALL
                    SELECT CONVERT(VARCHAR(100),F_CreateTime,23) F_CreateTime,F_ZoneID,F_Level,F_Pro,count(1) AS Num FROM 
                    GameCoreDB.dbo.T_RoleBaseData_2 WITH(NOLOCK)
                    WHERE F_UpdateTime<Dateadd(DAY,-3,GETDATE()) AND CONVERT(VARCHAR(100),F_CreateTime,23)>='{4}' AND CONVERT(VARCHAR(100),F_CreateTime,23)<='{5}'
                    group by CONVERT(VARCHAR(100),F_CreateTime,23),F_ZoneID,F_Level,F_Pro
                    UNION ALL
                    SELECT CONVERT(VARCHAR(100),F_CreateTime,23) F_CreateTime,F_ZoneID,F_Level,F_Pro,count(1) AS Num FROM 
                    GameCoreDB.dbo.T_RoleBaseData_3 WITH(NOLOCK)
                    WHERE F_UpdateTime<Dateadd(DAY,-3,GETDATE()) AND CONVERT(VARCHAR(100),F_CreateTime,23)>='{6}' AND CONVERT(VARCHAR(100),F_CreateTime,23)<='{7}'
                    group by CONVERT(VARCHAR(100),F_CreateTime,23),F_ZoneID,F_Level,F_Pro
                    UNION ALL
                    SELECT CONVERT(VARCHAR(100),F_CreateTime,23) F_CreateTime,F_ZoneID,F_Level,F_Pro,count(1) AS Num FROM 
                    GameCoreDB.dbo.T_RoleBaseData_4 WITH(NOLOCK)
                    WHERE F_UpdateTime<Dateadd(DAY,-3,GETDATE()) AND CONVERT(VARCHAR(100),F_CreateTime,23)>='{8}' AND CONVERT(VARCHAR(100),F_CreateTime,23)<='{9}'
                    group by CONVERT(VARCHAR(100),F_CreateTime,23),F_ZoneID,F_Level,F_Pro
                    )TEMP {10}
                    GROUP BY F_Level,F_Pro 
                    )
                    SELECT DISTINCT (a.F_Level) AS '等级',
			                    (SELECT ISNULL(SUM(F_Num), 0) FROM RVcte WHERE F_Level = a.F_Level AND F_Pro = 1) AS '猎魔者',
			                    (SELECT ISNULL(SUM(F_Num), 0) FROM RVcte WHERE F_Level = a.F_Level AND F_Pro = 2) AS '魔导师',
			                    (SELECT ISNULL(SUM(F_Num), 0) FROM RVcte WHERE F_Level = a.F_Level AND F_Pro = 3) AS '龙战士',
			                    (SELECT ISNULL(SUM(F_Num), 0) FROM RVcte WHERE F_Level = a.F_Level AND F_Pro = 4) AS '召唤师'
                    FROM RVcte a
                    ORDER BY a.F_Level ASC", searchdateB, searchdateE, searchdateB, searchdateE, searchdateB, searchdateE, searchdateB, searchdateE, searchdateB, searchdateE,sqlwhere);

            try
            {
                //string tmpl = "(select isnull(Sum(F_Num),0) from RVcte where F_Level=a.F_Level and F_Pro={0}) as '{1}'";
                ...
                GridView1.Columns.Add(new BoundField() { DataField = "等级", HeaderText = App_GlobalResources.Language.LblLevel, DataFormatString = App_GlobalResources.Language.Msg_RoleGradeFormat });
                columnValue.Add(0);
                foreach (KeyValuePair<string, string> item in MapVacationType1())
                {
        ...
        protected Dictionary<string, string> MapVacationType1()
        {
