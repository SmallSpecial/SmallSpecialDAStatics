...
{
    public partial class ItemQueryRank : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sqlStr = string.Format(@"SELECT UID,CID,count(1) as RCOUNT FROM {0:yyyy_MM_dd}_item_log ", searchdateB);
            ...
            if (tboxCID.Text.Trim().Length > 0)
            {
                ...
                sqlStr += @" and CID =" + tboxCID.Text + "";
            }

            if (ddlOPID.SelectedIndex>0)
            {
                sqlStr += @" and OPID=" + ddlOPID.SelectedValue + "";
            }
            if (ddlPara2Type.SelectedIndex > 0)
            {
                sqlStr += @" and PARA_2 = " + ddlPara2Type.SelectedValue + "";
            }
