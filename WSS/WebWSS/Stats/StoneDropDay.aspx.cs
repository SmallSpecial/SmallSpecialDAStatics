...
{
    public partial class StoneDropDay : Admin_Page
    {
...
FROM         T_EquipLevelVacation  where  F_Date >='" + searchdateB.ToString(SmallDateTimeFormat) + @"' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat) + @"' and F_SuitName='精炼' and F_EquipType='宝石' ";
...
";
            ...
            try
            {
                ...
                if (ControlChart1.Visible)
                {
                    ControlChart1.SetChart(GridView1, LabelTitle.Text.Replace(">>", " - ") + "  " + LabelArea.Text + " " + LabelTime.Text, ControlChartSelect1.State, true, 0, 1);
                }
            }
