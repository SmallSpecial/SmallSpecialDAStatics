...
{
    public partial class Count : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"SELECT convert(VARCHAR(10), F_Time,120) as 日期, sum(case F_Type when 0 then 1 else 0 end)  as 安装, sum(case F_Type when 1 then 1 else 0 end)  as 卸载, sum(case F_Type when 4 then 1 else 0 end)  as 下载 FROM T_CountDetail where F_Time>='" + searchdateB.ToString(SmallDateTimeFormat)   + "' and F_Time<='" + searchdateE.ToString(SmallDateTimeFormat)   + "' ";
            ...
            //}
            sql += "group by  convert(VARCHAR(10), F_Time,120) order by convert(VARCHAR(10), F_Time,120) desc";
