...
namespace WebWSS.Stats
{
    public partial class Test : BasePage
    {
        ...
        public void bind()
        {
            ...
            if (tboxRoleID.Text.Length>0)
            {
                sqlwhere += @" and F_RoleID=" + tboxRoleID.Text + "";
            }

            string sql = "";
            sql = @"SELECT distinct top 1  F_RoleName,F_RoleID FROM T_RoleLevelRank " + sqlwhere + @" ";
            ...
            try
            {
                ...
                {
                    lblerro.Text = dr["F_RoleID"] + "@@@<br/><hr/>";
                    if (lblDecoding.Visible)
                    {
                        string name1 = "";
                        if (lblDeType.Text=="100")
                        {
                            name1 = CodingTran.TranALL(dr["F_RoleName"].ToString());
                        }
                        else if (lblDeType.Text == "101")
                        {
                            name1 = CodingTran.TranALL1(dr["F_RoleName"].ToString());
                        }
                        else if (lblDeType.Text == "200")
                        {
                            name1 = CodingTran.TranALLALL(dr["F_RoleName"].ToString());
                        }
                        else if (lblDeType.Text == "300")
                        {
                            name1 = CodingTran.TranALLALLALL(dr["F_RoleName"].ToString());
                        }

                        if (tboxRoleName.Text.Length>0)
                        {
                            if (name1.IndexOf(tboxRoleName.Text) != -1)
                            {
                                lblerro.Text += name1;
                            }
                        }
                    ...
                    {
                        lblerro.Text += dr["F_RoleName"].ToString();
                    }
                    lblerro.Text += "<br/><hr/>";
                }
            }
