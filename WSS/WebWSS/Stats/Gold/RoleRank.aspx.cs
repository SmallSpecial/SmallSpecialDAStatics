...
namespace WebWSS.Stats.Gold
{
    public partial class RoleRank : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            if (Request.QueryString["p"] == "fuzhi")
                sqlwhere += " and F_GoldCount<0";
            else if (Request.QueryString["p"] == "zhengzhi")
                sqlwhere += " and F_GoldCount>0";
            else
                sqlwhere += " and F_PARA_1=" + Request.QueryString["opid"] + "";
            sqlwhere += " and F_OPID=" + Request.QueryString["opid"] + "";
            string sql="";
            sql = @"SELECT  top 100 ROW_NUMBER() OVER(order by F_GoldCount " + Request.QueryString["order"] + ") AS rownum, F_BigZone,F_ZoneID,F_UserID, F_RoleID, F_GoldCount,F_PickUpCount FROM T_Gold_GSLog_Rank with(nolock) where 1=1 " + sqlwhere + " ";
