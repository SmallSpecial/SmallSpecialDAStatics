...
{
    public partial class RoleLoseLevelPara : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"
with RVcte(F_Level, F_Pro, F_Num)
as
(
SELECT (F_Level-1)/5 as F_Level,F_Pro,Sum(F_Num) as F_Num FROM T_RoleLoseLevel  " + sqlwhere + @"
 group by (F_Level-1)/5,F_Pro 
)

select distinct(a.F_Level) as aa ,cast(F_Level*5+1 as varchar(3))+' - '+cast((F_Level+1)*5 as varchar(3)) as '"+App_GlobalResources.Language.LblLevel+@"',
{0}
 FROM RVcte a order by a.F_Level asc";
            ...
            try
            {
                ...
                string tmple = "(select isnull(Sum(F_Num),0) from RVcte where F_Level=a.F_Level and F_Pro={0}) as '{1}'";
                List<string> fields = new List<string>();
                foreach (KeyValuePair<string,string> item in MapVacationType())
                {
                    fields.Add(string.Format(tmple, item.Key, item.Value));
                }
                sql = string.Format(sql, string.Join(",", fields));
                ds = DBHelperDigGameDB.Query(sql);
                GridView1.Columns.Clear();
                GridView1.Columns.Add(new BoundField() { DataField = App_GlobalResources.Language.LblLevel, HeaderText = App_GlobalResources.Language.LblLevel, DataFormatString = App_GlobalResources.Language.Msg_RoleGradeFormat });
