...
{
    public partial class RoleLoseLevel : Admin_Page
    {
...
as
(
SELECT  F_Level,F_Pro,Sum(F_Num) as F_Num FROM T_RoleLoseLevel  " + sqlwhere + @"
 group by F_Level,F_Pro 
)
select distinct(a.F_Level) as '"+App_GlobalResources.Language.LblLevel+@"',
{0}  FROM RVcte a order by a.F_Level asc";
