...
{
    public partial class MoneyGoldRole : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"select Row_number() Over(order by getdate()) as number, rownum0,a.F_zoneid as F_zoneid0, F_GoldSum0, F_UserID0, F_RoleID0
,rownum1,b.F_zoneid as F_zoneid1,F_GoldSum1,  F_UserID1, F_RoleID1
,rownum2,c.F_zoneid as F_zoneid2,F_GoldSum2,  F_UserID2, F_RoleID2 from (
SELECT TOP 50 ROW_NUMBER() OVER(order by F_GoldSum desc) AS rownum0,F_zoneid,F_GoldSum as F_GoldSum0,F_UserID as F_UserID0,F_RoleID as F_RoleID0 FROM T_MoneyGoldRole where F_GoldType=0 " + sqlwhere + @" order by rownum0 asc
            ) a full outer join  (
SELECT TOP 50  ROW_NUMBER() OVER(order by F_GoldSum ASC) AS rownum1,F_zoneid,F_GoldSum as F_GoldSum1,F_UserID as F_UserID1,F_RoleID as F_RoleID1 FROM T_MoneyGoldRole where F_GoldType=1 " + sqlwhere + @" order by rownum1 asc
) b on a.rownum0=b.rownum1 full outer join (
SELECT TOP 50  ROW_NUMBER() OVER(order by F_GoldSum desc) AS rownum2,F_zoneid,F_GoldSum as F_GoldSum2,F_UserID as F_UserID2,F_RoleID as F_RoleID2 FROM T_MoneyGoldRole where F_GoldType=2 " + sqlwhere + @" order by rownum2 asc
) c on a.rownum0=c.rownum2  ";
