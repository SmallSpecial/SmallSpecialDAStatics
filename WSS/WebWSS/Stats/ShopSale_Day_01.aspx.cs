...
{
    public partial class ShopSale_Day_01 : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            if (queryDateView != null && queryDateView.Count > 0)
            {
                for (int i = 0; i < queryDateView.Count; i++)
                {
                    ...
                    inSqlBuilder.AppendFormat("['{0}']", queryDateView[i]["f_Date"].ToString());
                }
                string inSql = inSqlBuilder.ToString();
                string sql = @"SELECT F_ZoneID," + string.Join(",", times) + "FROM (SELECT F_BigZone, F_ZoneID,F_Date, SUM(F_TradeCount) AS F_TradeCount FROM T_ShopSale with(nolock) where F_ShopType=0 AND F_Date>='" + searchdateB.ToString(SmallDateTimeFormat) + "' and F_Date<='" + searchdateE.ToString(SmallDateTimeFormat) + "' ";

                if (DropDownListArea1.SelectedIndex > 0)
                {
                    sql += @" and F_BigZone=" + DropDownListArea1.SelectedValue.Split(',')[1] + "";
                }
                sql += " GROUP BY F_Date,F_BigZone,F_ZoneID) as TempData pivot (sum(F_TradeCount) for F_Date in (" + string.Join(",", times) + ")) as ourpivot order by F_ZoneID";
                try
                {
