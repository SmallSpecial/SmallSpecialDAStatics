...
{
    public partial class RoleLevelTime : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"with RVcte(F_Level,F_Pro, F_Num)
as
(
SELECT F_Level,F_Pro, AVG(F_Num) as F_Num FROM T_RoleLevelTime " + sqlwhere + @"
 group by F_Level,F_Pro 
)
select distinct(a.F_Level) as F_Level,
(select isnull(AVG(F_Num),0)  from RVcte where F_Level=a.F_Level and F_Pro=1) as '1',
(select isnull(AVG(F_Num),0)  from RVcte where F_Level=a.F_Level and F_Pro=2) as '2',
(select isnull(AVG(F_Num),0)  from RVcte where F_Level=a.F_Level and F_Pro=3) as '3',
(select isnull(AVG(F_Num),0)  from RVcte where F_Level=a.F_Level and F_Pro=4) as '4'
 ...
 FROM RVcte a order by a.F_Level asc";
            ...
            try
            {
                ...
                GridView1.Columns.Add(new BoundField() { DataField = "F_Level", HeaderText = App_GlobalResources.Language.LblLevel, DataFormatString =App_GlobalResources.Language.Msg_RoleGradeFormat });
                foreach (KeyValuePair<String,string> item in MapVacationType())
                {
                    GridView1.Columns.Add(new BoundField() { DataField =item.Key, HeaderText = item.Value});
                }
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (!lblerro.Visible)
                    {
                        ...
                        long sumy = 0;
                        long num = 0;
                        num = Convert.ToInt64(e.Row.Cells[1].Text);
                        if (num > 0)
                        {
                            sumy++;
                            sumi += num;
                        }
                        num = Convert.ToInt64(e.Row.Cells[2].Text);
                        if (num > 0)
                        {
                            sumy++;
                            sumi += num;
                        }
                        num = Convert.ToInt64(e.Row.Cells[3].Text);
                        if (num > 0)
                        {
                            sumy++;
                            sumi += num;
                        }
                        num = Convert.ToInt64(e.Row.Cells[4].Text);
                        if (num > 0)
                        {
                            sumy++;
                            sumi += num;
                        ...
                        {
                            e.Row.Cells[5].Text = (sumi / sumy).ToString("#,#0");
                        }
            ...
            {
                lblinfo.Text = "ERRPOR:" + ex.Message;
