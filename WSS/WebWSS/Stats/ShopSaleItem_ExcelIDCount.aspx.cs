...
{
    public partial class ShopSaleItem_ExcelIDCount : Admin_Page
    {
...
where 1=1 AND F_Date='" + searchdateB.ToString(SmallDateTimeFormat)   + "'";
            ...
            if (tboxExcelName.Text != "" && tboxExcelName.Text.Length > 0)
            {
                ...
                {
                    LabelTime.Text = searchdateB.ToString(SmallDateTimeFormat)   + " " + tboxExcelName.Text+"消耗";
                    sql += @" and F_ItemExcelID=" + excelID + "";
                }

            }
            sql += " order by F_UserID,F_RoleID";
