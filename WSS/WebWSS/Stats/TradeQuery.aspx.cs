...
{
    public partial class TradeQuery : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            {
                sqlwhere += @" and PARA_1 = " + tboxPARA_1.Text.Trim() + "";
            }

            if (tboxPARA_2.Text.Trim().Length > 0)
            {
                sqlwhere += @" and PARA_2 = " + tboxPARA_2.Text.Trim() + "";
            }
            ...
            sqlwhere += " order by " + (string)ViewState["SortOrder"] + " " + (string)ViewState["OrderDire"];
        ...
        public string GetOPBakFormate(string value)
        {
            try
            {
                ...
                if (items.Length == 5)
                {
                    return String.Format("类型:{0} 交易ID:{1} GUID:{2} 道具:{3} 叠加:{4}", items[0], items[1], items[2], GetTextName(items[3].ToString()), items[4]);
                }
                else if (items.Length == 4)
                {
                    return String.Format("类型:{0} 交易ID:{1} 金钱:{2} 元宝:{3}", items[0], items[1], items[2], items[3]);
                }
                return value;
        ...
        public string GetTextName(string value)
        {
            try
            {
                ...
                string sql = @"SELECT ( cast(F_ExcelID as varchar(9)) +' '+F_Name ) as F_Name  FROM T_BaseGameName with(nolock) WHERE (F_ExcelID = " + value + ") ";
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                else if (e.Row.RowType==DataControlRowType.Header)
                {
                    ...
                    {
                        if (field.SortExpression.Equals(ViewState["SortOrder"]))
                        {
                            Label lbl=new Label();
                            lbl.Text = ViewState["OrderDire"].Equals("ASC") ? "▲" : "▼";
                            e.Row.Cells[GridView1.Columns.IndexOf(field)].Controls.Add(lbl);
                        }
                    }
                }
