...
{
    public partial class ShopBaseItemImport : Admin_Page, ICallbackEventHandler
    {
        ...
        public bool AddItem(DataRow dr, string bigzoneid, string zoneid)
        {
            ...
            string t_ITEMPRICE = dr["F_ITEMPRICE"].ToString().Trim();


            string strTips = "";
            if (!Common.Validate.IsInt(t_EXCELID))
            {
                strTips = "EXCEL编号填写不正确!!!";

            }
            else if (!Common.Validate.IsInt(t_ITEMPRICE))
            {
                strTips = "参考价格填写不正确!!!";
            ...
            //开始新增
            string sqlStr = "insert into OPENQUERY([LKSV] ,'select * from gameshop_itembase limit 0,1') (F_EXCELID,F_SHOPTYPE,F_ITEMTYPE,F_ITEMPRICE) values (" + t_EXCELID + "," + t_SHOPTYPE + "," + t_ITEMTYPE + "," + t_ITEMPRICE + ") ";
            return ExecSql(sqlStr, bigzoneid, zoneid);
        ...
        protected void btnPublish_Click(object sender, EventArgs e)
        {
            try
            {
                        ...
                        if (cblTemp.SelectedIndex != -1)
                        {
                            for (int x = 0; x < cblTemp.Items.Count; x++)
                            {
                                if (cblTemp.Items[x].Selected)
                                {
                                    ...
                                    if (cbReset.Checked)
                                    {
                                        string sql = "DELETE FROM OPENQUERY([LKSV] ,'select * from gameshop_itembase ')";
                ...
                //LblMessagePub.Text += string.Format(".... {0} 条成功 {1} 条失败", count - countLose, countLose);
