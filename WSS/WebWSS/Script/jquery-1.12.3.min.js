...
(function ($) {
    $.format = function (str, args) {
        ...
        if (arguments.length > 0) {
            if (arguments.length == 1 && typeof (args) == "object") {//由于这是使用jq扩展方法，而不是string属性扩展，此处语句无效
                for (var key in args) {
            ...
            else if (arguments.length == 2 && typeof (arguments[1]) == "object")//jq扩展此处才是正式开始执行
            {//
                for (var key in args) {
                    var reg = new RegExp("({" + key + "})", "g");
                    result = result.replace(reg, args[key]);
    ...
    $.stringconverDate=function(milStr){
        var dt = milStr.replace('/Date(', '').replace(')/', '');
        var t = new Date(parseInt(dt));
        return t;
    },
    $.getDateString = function (dateVal) {
      return  dateVal.getFullYear() + "-" + (dateVal.getMonth() + 1) + "-" + dateVal.getDate() + " " + dateVal.getHours() + "：" + dateVal.getMinutes();
    },
    $.milDateString = function (milStr) {
        var date = $.stringconverDate(milStr);
