...
{
    public partial class ShopInfoLog : Admin_Page
    {
        #region 属性
        ...
        public void bind()
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                if (i == 0)
                    sql += string.Format("SELECT \"{0}\" AS DateTime,PARA_1,SUM(SUBSTRING_INDEX(OP_BAK,\"	\",1)) SumNum,COUNT(PARA_1) CountNum FROM {1}_item_log WHERE OPID = 10047 AND PARA_2 = 15 {2} GROUP BY PARA_1", strSTime, strSTime, sqlWhere);
                else
                    sql += string.Format(" UNION ALL SELECT \"{0}\" AS DateTime,PARA_1,SUM(SUBSTRING_INDEX(OP_BAK,\"	\",1)) SumNum,COUNT(PARA_1) CountNum FROM {1}_item_log WHERE OPID = 10047 AND PARA_2 = 15 {2} GROUP BY PARA_1", strSTime, strSTime, sqlWhere);
            }
