...
{
    public partial class TenEvenDraw : Admin_Page
    {
        ...
        public string GetOPBakFormate(string value)
        {
            try
            {
                ...
                return String.Format("用户ID:{0} 角色ID:{1} 抽奖ID:{2} 抽奖类型:{3} 花费的金钱：{4}", items[0], items[1], GetAward(items[2]), GetAwardType(items[3]), items[4]);
            }
            catch (System.Exception ex)
            {
                return value;
        ...
        protected string GetAwardType(string value)
        {
            switch (value)
            {
                case "0":
        ...
        protected string GetAward(string value)
        {
            switch(value)
            {
                case "1001":
