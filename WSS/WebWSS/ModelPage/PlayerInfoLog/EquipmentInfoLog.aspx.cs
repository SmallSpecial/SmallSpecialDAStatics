...
{
    public partial class EquipmentInfoLog : Admin_Page
    {
        #region 属性
        ...
        public void bind()
        {
            ...
            string strOPID = ddlOPID.SelectedValue;//操作类型
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                if (i == 0)
                    sql += string.Format("SELECT \"{0}\" AS DateTime,COUNT(DISTINCT CID) AS PeopleNum,COUNT(CID) AS Num FROM {1}_other_log WHERE OPID={2}", strSTime, strSTime, strOPID);
                else
                    sql += string.Format(" UNION ALL SELECT \"{0}\" AS DateTime,COUNT(DISTINCT CID) AS PeopleNum,COUNT(CID) AS Num FROM {1}_other_log WHERE OPID={2}", strSTime, strSTime, strOPID);
            }
