...
{
    public partial class RechargeRecord :Admin_Page
    {
        #region 属性
        //获取DigGameDB连接字符串
        ...
        public void Bind()
        {
            ...
            string sql = string.Format("SELECT F_ID,F_GlobalID,F_UserID,F_TransactionID,F_WebRequestResult,F_ReturnCode,F_ReturnStr,F_ProductID,F_InsertTime,F_Price,F_DepositResult,F_StoreName,F_ZoneID,F_ChannelID,F_PlayerLevel FROM T_Deposit_Verify_Result_Log WHERE 1=1 AND CONVERT(VARCHAR(100),F_InsertTime,23)>='{0}' AND CONVERT(VARCHAR(100),F_InsertTime,23)<='{1}' {2}", searchdateB.ToString("yyyy-MM-dd"), searchdateE.ToString("yyyy-MM-dd"), sqlWhere);
            try
            {
