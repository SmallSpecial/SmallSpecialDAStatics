...
{
    public partial class RedDiamondGiftShop : Admin_Page
    {
        #region 属性
        ...
        public void bind()
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                if (i == 0)
                    sql += string.Format("SELECT \"{0}\" AS DateTime,PARA_1 ProductName,SUM(PARA_2) SoldNum,COUNT(CID) TradeNum,COUNT(DISTINCT CID) RoleNum,SUM(SUBSTRING_INDEX(SUBSTRING_INDEX(OP_BAK,\"	\",6),\"	\",-1)) MoneyNum FROM {1}_other_log WHERE (OPID=40004 OR OPID=40005) AND PARA_1<>0 AND PARA_1<>1 AND SUBSTRING_INDEX(SUBSTRING_INDEX(OP_BAK,\"	\",3),\"	\",-1)=1 GROUP BY PARA_1", strSTime, strSTime);
                else
                    sql += string.Format(" UNION ALL SELECT \"{0}\" AS DateTime,PARA_1 ProductName,SUM(PARA_2) SoldNum,COUNT(CID) TradeNum,COUNT(DISTINCT CID) RoleNum,SUM(SUBSTRING_INDEX(SUBSTRING_INDEX(OP_BAK,\"	\",6),\"	\",-1)) MoneyNum FROM {1}_other_log WHERE (OPID=40004 OR OPID=40005) AND PARA_1<>0 AND PARA_1<>1 AND SUBSTRING_INDEX(SUBSTRING_INDEX(OP_BAK,\"	\",3),\"	\",-1)=1 GROUP BY PARA_1", strSTime, strSTime);
            }
