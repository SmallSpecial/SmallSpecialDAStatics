...
{
    public partial class SoldOutLog : Admin_Page
    {
        ...
        public void bind()
        {
            ...
            string sqlwhere = " where 1=1 and OPID=50125";
        ...
        public string GetOPBakFormate(string value)
        {
            try
            {
                ...
                if (PageLanguage == "ko-kr")
                {
                    return String.Format("상품 GlobalID:{0} 상품 Key:{1} 상품 Excel ID:{2} 상품 Number:{3} 상품 판매 재화 유형：{4} 진영 작업 시간：{5}", items[0], items[1], items[2], items[3], items[4], items[5]);
                }
                else
                {
                    return String.Format("商品GlobalID:{0} 商品唯一Key:{1} 商品ExcelID:{2} 商品Number:{3} 商品出售货币类型：{4} 阵营操作时间：{5}", items[0], items[1], items[2], items[3], items[4], items[5]);
                }
