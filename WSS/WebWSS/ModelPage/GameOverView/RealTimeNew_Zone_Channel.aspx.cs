...
{
    public partial class RealTimeNew_Zone_Channel : Admin_Page
    {
        #region 属性
        //获取DigGameDB连接字符串
        ...
        public void Bind()
        {
            ...
            //角色信息
            string sqlRoleInfo = string.Format("SELECT F_ChannelID Channel,F_ZoneID ZoneID,COUNT(DISTINCT F_UserID) RoleUserNum,COUNT(DISTINCT F_DeviceID) RoleDeviceNum,COUNT(F_RoleID) RoleNum FROM(SELECT F_UserID,F_RoleID,F_DeviceID,F_ChannelID,F_ZoneID FROM T_RoleBaseData_0 WHERE CONVERT(VARCHAR(100),F_CreateTime,23)='{0}' {1} UNION ALL SELECT F_UserID,F_RoleID,F_DeviceID,F_ChannelID,F_ZoneID FROM T_RoleBaseData_1 WHERE CONVERT(VARCHAR(100),F_CreateTime,23)='{0}' {1} UNION ALL SELECT F_UserID,F_RoleID,F_DeviceID,F_ChannelID,F_ZoneID FROM T_RoleBaseData_2 WHERE CONVERT(VARCHAR(100),F_CreateTime,23)='{0}' {1} UNION ALL SELECT F_UserID,F_RoleID,F_DeviceID,F_ChannelID,F_ZoneID FROM T_RoleBaseData_3 WHERE CONVERT(VARCHAR(100),F_CreateTime,23)='{0}' {1} UNION ALL SELECT F_UserID,F_RoleID,F_DeviceID,F_ChannelID,F_ZoneID FROM T_RoleBaseData_4 WHERE CONVERT(VARCHAR(100),F_CreateTime,23)='{0}' {1} )TEMP GROUP BY F_ChannelID,F_ZoneID", searchdateB.ToString("yyyy-MM-dd"), sqlWhere);
            //充值信息
            string sqlRechargeInfo = string.Format("SELECT F_ChannelID ChannelID,F_ZoneID ZoneID,SUM(F_Price) RechargeAmount,COUNT(DISTINCT F_UserID) RechargePeoleOfNum FROM T_Deposit_Verify_Result_Log WHERE F_DepositResult=1 AND CONVERT(VARCHAR(100),F_InsertTime,23)='{0}' {1} GROUP BY F_ChannelID,F_ZoneID", searchdateB.ToString("yyyy-MM-dd"), sqlWhere);
            try
            {
                DataTable dt = new DataTable();
                dt.Columns.Add("ZoneID", System.Type.GetType("System.String"));
                ...
                int num = 0;
                int num1 = dsRoleInfo.Tables[0].Rows.Count;
                int num2 = dsRechargeInfo.Tables[0].Rows.Count;
                num = num1 > num2 ? num1 : num2;
                
                if (num == dsRoleInfo.Tables[0].Rows.Count)
                {
                    for (int i = 0; i < dsRoleInfo.Tables[0].Rows.Count; i++)
                    {
                        ...
                        //角色信息
                        newRow["ZoneID"] = dsRoleInfo.Tables[0].Rows[i]["ZoneID"];
