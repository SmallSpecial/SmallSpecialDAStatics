...
{
    public partial class MoneyGoldRole : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                tboxTimeB.Text = DateTime.Now.AddDays(-1).ToShortDateString();
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"select * from 
(SELECT TOP 50 ROW_NUMBER() OVER(order by F_GoldSum desc) AS rownum0,F_GoldSum as F_GoldSum0,F_UserID as F_UserID0,F_RoleID as F_RoleID0 FROM T_MoneyGoldRole where F_GoldType=0 " + sqlwhere + " ) a left join (SELECT TOP 50  ROW_NUMBER() OVER(order by F_GoldSum ASC) AS rownum1,F_GoldSum as F_GoldSum1,F_UserID as F_UserID1,F_RoleID as F_RoleID1 FROM T_MoneyGoldRole where F_GoldType=1 " + sqlwhere + " ) b on a.rownum0=b.rownum1 left join (SELECT TOP 50  ROW_NUMBER() OVER(order by F_GoldSum desc) AS rownum2,F_GoldSum as F_GoldSum2,F_UserID as F_UserID2,F_RoleID as F_RoleID2 FROM T_MoneyGoldRole where F_GoldType=2 " + sqlwhere + " ) c on b.rownum1=c.rownum2";
