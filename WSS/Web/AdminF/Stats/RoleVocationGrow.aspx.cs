...
{
    public partial class RoleVocationGrow : System.Web.UI.Page
    {
...
as
(
SELECT convert(varchar(10),F_Date,120),F_VocationType, Sum(F_VocationNum) as F_VocationNum FROM T_RoleVocationGrow " + sqlwhere + @"
 group by F_Date,F_VocationType 
)
select distinct(a.F_Date) as 日期,
(select isnull(Sum(F_VocationNum),0) from RVcte where F_Date=a.F_Date and F_VocationType/6=6) as 虎贲,
...
(select isnull(Sum(F_VocationNum),0) from RVcte where F_Date=a.F_Date and F_VocationType/6=7) as 气功师,
...
(select isnull(Sum(F_VocationNum),0) from RVcte where F_Date=a.F_Date and F_VocationType/6=5) as 行者
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowIndex >= 0 && !lblerro.Visible)
                {
                    ...
                    sum1 += Convert.ToInt64(e.Row.Cells[2].Text);
                    sum1 += Convert.ToInt64(e.Row.Cells[3].Text);
                    sum1 += Convert.ToInt64(e.Row.Cells[4].Text);
                    sum1 += Convert.ToInt64(e.Row.Cells[5].Text);
                    sum1 += Convert.ToInt64(e.Row.Cells[6].Text);
                    sum1 += Convert.ToInt64(e.Row.Cells[7].Text);
                    sum1 += Convert.ToInt64(e.Row.Cells[8].Text);
                    e.Row.Cells[9].Text = sum1.ToString("#,#0");
                }
