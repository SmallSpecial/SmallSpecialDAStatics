...
{
    public partial class RoleVocationLevelPara : System.Web.UI.Page
    {
...
as
(
SELECT (F_Level-1)/5 as F_Level,F_VocationType, Sum(F_VocationNum) as F_VocationNum FROM T_RoleLevel " + sqlwhere + @"
 group by (F_Level-1)/5,F_VocationType 
)
select distinct(a.F_Level) as aa ,cast(F_Level*5+1 as varchar(3))+' - '+cast((F_Level+1)*5 as varchar(3)) as 等级,
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowIndex >= 0 && !lblerro.Visible)
                {
                    ...
                    sum9 += Convert.ToInt64(e.Row.Cells[9].Text.Replace(",", ""));
                }
