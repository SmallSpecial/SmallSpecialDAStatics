...
{
    public partial class ItemDropDay : System.Web.UI.Page
    {
        string ConnStrDigGameDB = System.Configuration.ConfigurationManager.AppSettings["ConnectionStringDigGameDB"];
        string ConnStrGSSDB = System.Configuration.ConfigurationManager.AppSettings["ConnectionStringGSSDB"];
        string DateTimeFormat = System.Configuration.ConfigurationManager.AppSettings["DateTimeFormat"];
...
FROM         T_EquipStarVacation  where cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime) >='" + searchdateB.ToString(DateTimeFormat) + @"' and cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime)<='" + searchdateE.ToString(DateTimeFormat) + @"' ";
            ...
            sql += @")
select '" + searchdateB.ToShortDateString() + " 至 " + searchdateE.ToShortDateString() + @"' as 日期, 
...
(select isnull(sum(F_ItemCount),0) from T_EquipStarVacation_CTE where F_StarLevel=a.F_StarLevel and left(F_Vocation,3)='气功师') as 气功师,
...
FROM         T_EquipLevelVacation  where cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime) >='" + searchdateB.ToString(DateTimeFormat) + @"' and cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime)<='" + searchdateE.ToString(DateTimeFormat) + @"' ";
            if (DropDownListArea1.SelectedIndex > 0)
            {
                sql += @" and F_BigZone=" + DropDownListArea1.SelectedValue.Split(',')[1] + "";
            }
