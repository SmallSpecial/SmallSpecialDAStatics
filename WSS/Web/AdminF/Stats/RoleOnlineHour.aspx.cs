...
{
    public partial class RoleOnlineHour : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            ...
            string sql = "";
            sql = @"select F_Hour,sum(F_OnlineNum) as F_OnlineNum,sum(F_OnlineTime)/60 as F_OnlineNumACU,sum(F_OnlineIpNum) as F_OnlineIpNum, sum(F_OnlineTime) as F_OnlineTime, sum(F_LoginNum) as F_LoginNum, sum(F_LoginIpNum) as F_LoginIpNum,sum(F_ExitNum) as F_ExitNum,sum(F_ExitIpNum) as F_ExitIpNum from T_UserOnlineBaseDig where 1=1 " + sqlwhere + "  group by F_Hour";
