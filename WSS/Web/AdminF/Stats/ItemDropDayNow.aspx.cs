...
{
    public partial class ItemDropDayNow : System.Web.UI.Page
    {
...
FROM         T_EquipStarVacation  where cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime) >='" + searchdateB.ToShortDateString() + @"' and cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime)<='" + searchdateE.ToShortDateString() + @"' ";
...
FROM         T_EquipLevelVacation  where cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime) >='" + searchdateB.ToShortDateString() + @"' and cast(cast(F_Year as varchar(4))+'-'+cast(F_Month as varchar(2))+'-'+cast(F_Day as varchar(2)) as datetime)<='" + searchdateE.ToShortDateString() + @"' ";
            if (DropDownListArea1.SelectedIndex > 0)
            {
                sql += @" and F_BigZone=" + DropDownListArea1.SelectedValue.Split(',')[1] + "";
            }
