...
function isaspx(num)
{
    var arr1 = new Array("defaultlist","offerlist","machininglist","investmentlist","joblist","shoplist",
                        "userlist","companylist","newslist","servicelist","personlist","exhibitionlist",
                        "brandlist","searchlist","keywordlist","cplist","csslist");
    var arr2 = new Array("default","offer","machining","investment","job","shop","user","company","news",
                        "service","person","exhibition","brand","search","keyword","cp","css");
    var arr3 = new Array("","offer","machining","investment","job","shop","user","company","news",
                        "service","person","exhibition","brand","search","keyword","cp","css");
    for(i=0;i<arr1.length;i++)
    {
        $(arr1[i]).style.display = "none";
        $(arr2[i]).className = "two title02";
    }
    $(arr1[num-1]).style.display = "block";
    $(arr2[num-1]).className = "one title01";
...
{
    var chkall = document.all["chkallaspx"];
    var chkother;
...
function createhtml()
{
	...
	{
	    alertnotbut("<img src=\"../images/ajax-loader.gif\" /><br>系统正在为入库的模版生成文件,这可能需要一些时间",false);
	    
		createtemps(tempnames,document.getElementById("hidT_ID").value,document.getElementById("hidpath").value);
	}
}
function createAll(tid,path)
{
     alertnotbut("<img src=\"../images/ajax-loader.gif\" /><br>系统正在为入库的模版生成文件,这可能需要一些时间",false);
     createtemps("all|",tid,path);
}

function createtemps(tempnames,tid,path)
{
    var url = "AddTemplate.aspx?TempNames="+tempnames+"&tid="+tid+"&path="+path
...
function createmobulehtml()
{
	...
	{
	    alertnotbut("<img src=\"../images/ajax-loader.gif\" /><br>系统正在为入库的模版生成文件,这可能需要一些时间");
...
function deletehtml()
{
	...
	{
	    alertnotbut("<img src=\"../images/ajax-loader.gif\" /><br>系统正在删除模版文件,这可能需要一些时间");
...
function ShopTemplatesSetting(template,folder)
{
    var strset = "";
    var dWidth = 500;
    var dHeight = 300;    
    ...
    $("window").src = "ShopTemplateSetting.aspx?template="+template+"&folder=" + folder;
    $("window").style.width = dWidth + "px";
    $("window").style.display = "block";
    $("window").style.height = dHeight + "px";
