function UploadImage(objName) {
    ...
    this.Show = function(id,objIndex) {
        ...
        $("uploadfrm").src = "/Common/UploadImage." + config.Suffix + "?TabName=" + $F("Upload_TabName") + "&iswatermark=" + $F("Upload_IsWaterMark") + "&AtID=" + id + "&tmp=" + Math.random();
...
function UploadFile(objName) {
    ...
    this.Show = function(id,objIndex) {
        ...
        var x = Math.round(pageSize.width/2) - (dWidth /2) + scrollPos.scrollX;
	    var y = Math.round(pageSize.height/2) - (dHeight /2) + scrollPos.scrollY;        
        $("_uploadfrm").src = "/Common/UploadImage." + config.Suffix + "?filetype=file&TabName=" + $F("_Upload_TabName") + "&AtID=" + id + "&tmp=" + Math.random();
