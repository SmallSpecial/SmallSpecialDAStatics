...
namespace GSSUI.AClass
{
    public class TaskMenu
    {
        ...
        //[DllImport("kernel32.dll", EntryPoint = "ModifyStyle", CharSet = CharSet.Auto)]
        //public static extern bool ModifyStyle(int removM, int addM, int pos);
        ...
        public static void DeleteSYSMENU(Form form)
        {
            int windowLong = (GetWindowLong(new HandleRef(form, form.Handle), -16));
            SetWindowLong(new HandleRef(form, form.Handle), -16, windowLong & (~WS_SYSMENU));
            InitSYSMENU(form);
        }
        public static void AddSYSMENU(Form form)
        {
            DeleteSYSMENU(form);
            int windowLong = (GetWindowLong(new HandleRef(form, form.Handle), -16));
            SetWindowLong(new HandleRef(form, form.Handle), -16, windowLong | WS_SYSMENU );
