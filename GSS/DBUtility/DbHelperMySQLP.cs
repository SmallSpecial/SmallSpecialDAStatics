...
namespace GSS.DBUtility
{
    /// <summary>
    /// 数据访问抽象基础类
    /// Copyright (C) ShenlongyouSoft
    /// </summary>
    public class DbHelperMySQLP
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString = "";
        public DbHelperMySQLP()
        {
        }
        public DbHelperMySQLP(string ConnectionString)
        {
        ...
        public string RunNoQueryProcedure(string procedureName, params MySqlParameter[] param)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    //  edit hexw 2017-9-6 result 由zoneroledatadb_TakeRoleData存储过程返回 查询角色情况 状态码,当前时间状态码都是0
                    return true.ToString();
                }
            }
            catch (Exception ex)
