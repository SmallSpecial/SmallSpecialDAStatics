...
namespace UserControlDLL
{
    public partial class ComCheckBoxList : UserControl
    {
        ...
        public ArrayList GetItemSelectedValue()
        {
            return GetCheckedItemsValue();
        }

        public ArrayList GetCheckedItemsValue()
        {
            ArrayList result = new ArrayList();
            int count = this.checkListBox.Items.Count;
            for (int i = 0; i < count; i++)
            {
                if (this.checkListBox.GetItemChecked(i))
                {
                    this.checkListBox.SetSelected(i, true);
                    result.Add(this.checkListBox.SelectedValue.ToString());
                }
            }
            return result;
        }
